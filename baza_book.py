...
class Database():
	...
	# def select_products_for_category_file_name(self,file_name):
	# 	self.cur.execute(f"SELECT * FROM products WHERE file_name = '{file_name}' ")
	# 	return self.cur.fetchall()
	...
	def insert_products(self,file_name,category_id,file_des,file_muallif,file_yili,file_narxi,file_id,file_photo):
		self.cur.execute(f"""INSERT INTO products(file_name,category_id,file_des,file_muallif,file_yili,file_narxi,file_id,file_photo) values("{file_name}",{category_id},"{file_des}","{file_muallif}","{file_yili}","{file_narxi}","{file_id}","{file_photo}")""")
		return self.conn.commit()
